...
| Image generation | `POST /api/image/generate`, `GET /api/image/status/{job_id}`, `GET /api/image/recent` | Turn stream image prompts with manual/auto-generate controls, Campaign tab scene images | `tests/backend/test_image_api.py`, `tests/frontend/flows/dtm_image_flow.test.ts` |
...
| GPU stats | `GET /api/gpu-stats` (nvidia-smi + Ollama `/api/ps`, 5s server-side cache) | Sidebar GPU widget with load/VRAM/temp bars + model tags (15s polling) | `tests/backend/test_image_api.py` |
| GPU memory orchestration | Automatic Ollama ↔ image-gen VRAM swap when `tge_completion_mode=ollama` and `image_backend` is `diffusers` or `comfyui`. Evicts Ollama before generation, unloads image model (diffusers) and reloads Ollama after. | N/A (transparent) | `tests/backend/test_image_api.py` |
